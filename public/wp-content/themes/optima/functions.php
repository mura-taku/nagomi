...
if (!function_exists('gt3_get_pf_type_output')) {
	function gt3_get_pf_type_output($pf, $width, $height, $featured_image)
	{
        ...
        $featured_standard = '<div class="blog_post_media"><img src="' . esc_url($featured_image[0]) . '" alt="'. esc_html__('featured_image', 'optima'). '" /></div>';
...
if (!function_exists('gt3_get_page_title')) {
    function gt3_get_page_title($id) {
        ...
        if ($page_title_conditional == 'yes') {
            ...
            $page_title_height = (is_array($page_title_height) && !empty($page_title_height['height'])) ? $page_title_height['height'] : '';
            $header_height = gt3_option('header_height');
            $header_height = (is_array($header_height) && !empty($header_height['height'])) ? $header_height['height'] : '';
...
if (!function_exists('gt3_get_logo')) {
    function gt3_get_logo() {
        ...
        $logo_height = (is_array($logo_height) && !empty($logo_height['height'])) ? $logo_height['height'] : '';
        $logo_max_height = gt3_option('logo_max_height');
        $sticky_logo_height = gt3_option('sticky_logo_height');
        $sticky_logo_height = (is_array($sticky_logo_height) && !empty($sticky_logo_height['height'])) ? $sticky_logo_height['height'] : '';
...
if(apply_filters('gt3_booked_styling_disable', false)) {
	add_filter('admin_body_class', function($classes){
		return $classes . ' gt3_booked_styling_disable';
	});
}
